...
class TextClusterSummarizer:
    ...
    def local_cluster_embeddings(self, embeddings: np.ndarray, dim: int, num_neighbors: int = 10,
                                 metric: str = "cosine") -> np.ndarray:
        return umap.UMAP(n_neighbors=num_neighbors, n_components=dim, metric=metric).fit_transform(embeddings)

    def get_optimal_clusters(self, embeddings: np.ndarray, max_clusters: int = 50, random_state: int = None) -> int:
    ...
    def GMM_cluster(self, embeddings: np.ndarray, threshold: float, random_state: int = None):
        if random_state is None:
            random_state = self.random_seed
    ...
    def recursive_embed_cluster_summarize(self, texts: List[str], level: int = 1, n_levels: int = 3) -> Dict[
    ...
    def run(self):
        leaf_texts = self.load_and_split_documents()
