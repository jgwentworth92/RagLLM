...
class AgenticChunker:
    ...
    def add_proposition(self, proposition):
        ...
        if len(self.chunks) == 0:
            if self.print_logging:
                log.info("No chunks, creating a new one")
        ...
        if chunk_id:
            if self.print_logging:
                log.info(f"Chunk Found ({self.chunks[chunk_id]['chunk_id']}), adding to: {self.chunks[chunk_id]['title']}")
    ...
    def _create_new_chunk(self, proposition):
        ...
        if self.print_logging:
            log.info(f"Created new chunk ({new_chunk_id}): {new_chunk_title}")
    ...
    def pretty_print_chunks(self):
        ...
        for chunk_id, chunk in self.chunks.items():
            log.info(f"Chunk #{chunk['chunk_index']}")
            log.info(f"Chunk ID: {chunk_id}")
            log.info(f"Summary: {chunk['summary']}")
            log.info(f"Propositions:")
            for prop in chunk['propositions']:
                log.info(f"    -{prop}")
            log.info("\n\n")

    def pretty_print_chunk_outline(self):
        log.info("Chunk Outline\n")
        log.info(self.get_chunk_outline())